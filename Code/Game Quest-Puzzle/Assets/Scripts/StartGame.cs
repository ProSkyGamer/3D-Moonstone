...
public class StartGame : MonoBehaviour
{
    ...
    [SerializeField] GameObject[] all_puzzles;
    [SerializeField] GameObject[] all_hints;

    void Start()
    {
        ResetTime();
        if(PlayerPrefs.GetString("reset_prefs")!="yes")
        {
            PlayerPrefs.DeleteAll();
            PlayerPrefs.SetString("reset_prefs", "yes");
            print(PlayerPrefs.GetString("reset_prefs"));
        }
        ...
        else if(PlayerPrefs.GetString("is_started_dialog")=="true")
        {
            NeededStartNewDialogue();
        }
        if (PlayerPrefs.GetInt("dialogues_number") > 0)
        {
    ...
    {
        if (PlayerPrefs.GetInt("game_border_z") == 0)
        {
            PlayerPrefs.SetInt("game_border_z", 100);
            Camera.main.GetComponent<JoysticMovement>().Start();
        }
        if (PlayerPrefs.GetString("need_to_complete_findsibling") != "true" && PlayerPrefs.GetString("need_to_complete_followingpuzzle") != "true"
            && PlayerPrefs.GetString("need_to_complete_plates") != "true" && PlayerPrefs.GetString("need_to_complete_findobject") != "true")
        {
            if (PlayerPrefs.GetString("opened_findsibling") == "true")
            {
                all_puzzles[0].GetComponent<Outline>().enabled = true;
                all_puzzles[0].GetComponent<ClickEvent>().enabled = true;

            }
            if (PlayerPrefs.GetString("opened_followingpuzzle") == "true")
            {
                all_puzzles[1].GetComponent<Outline>().enabled = true;
                all_puzzles[1].GetComponent<ClickEvent>().enabled = true;
                
            }
            if (PlayerPrefs.GetString("opened_plates") == "true")
            {
                all_puzzles[2].GetComponent<Outline>().enabled = true;
                all_puzzles[2].GetComponent<ClickEvent>().enabled = true;
                
            }
            if (PlayerPrefs.GetString("opened_findobject") == "true")
            {
                all_puzzles[3].GetComponent<Outline>().enabled = true;
                all_puzzles[3].GetComponent<ClickEvent>().enabled = true;
                
            }
        }
        else
        {
            if (PlayerPrefs.GetString("need_to_complete_findsibling") == "true")
            {
                all_hints[0].SetActive(true);
                all_hints[1].SetActive(true);
            }
            else if (PlayerPrefs.GetString("need_to_complete_followingpuzzle") == "true")
            {
                all_puzzles[0].GetComponent<Outline>().enabled = false;
                all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
                all_puzzles[1].GetComponent<Outline>().enabled = true;
                all_puzzles[1].GetComponent<ClickEvent>().enabled = true;
                all_hints[0].SetActive(true);
                all_hints[2].SetActive(true);
            }
            else if (PlayerPrefs.GetString("need_to_complete_plates") == "true")
            {
                all_puzzles[0].GetComponent<Outline>().enabled = false;
                all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
                all_puzzles[2].GetComponent<Outline>().enabled = true;
            ...
            else if (PlayerPrefs.GetString("need_to_complete_findobject") == "true")
            {
                all_puzzles[0].GetComponent<Outline>().enabled = false;
                all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
                all_puzzles[3].GetComponent<Outline>().enabled = true;
    ...
    private void Update()
    {
        if (need && Input.GetMouseButton(0))
        {
            if (game == "findsibling")
            {
                ...
                if (PlayerPrefs.GetString("need_to_complete_findsibling") == "true")
                {
                    PlayerPrefs.SetString("need_to_complete_findsibling", "false");
                    all_hints[1].SetActive(false);
                    all_hints[0].SetActive(false);
                    PlayerPrefs.SetString("opened_findsibling", "true");
                    CheckPuzzles();
                    NeededStartNewDialogue();
                }
            ...
            else if (game == "plates")
            {
                ...
                swiping.SetActive(true);
                if (PlayerPrefs.GetString("need_to_complete_plates") == "true")
                {
                    PlayerPrefs.SetString("need_to_complete_plates", "false");
                    all_hints[3].SetActive(false);
                    all_hints[0].SetActive(false);
                    PlayerPrefs.SetString("opened_plates", "true");
                    CheckPuzzles();
                    NeededStartNewDialogue();
                }
            ...
            else if (game == "followingplates")
            {
                ...
                swiping.SetActive(true);
                if (PlayerPrefs.GetString("need_to_complete_followingpuzzle") == "true")
                {
                    PlayerPrefs.SetString("need_to_complete_followingpuzzle", "false");
                    all_hints[2].SetActive(false);
                    all_hints[0].SetActive(false);
                    PlayerPrefs.SetString("opened_followingpuzzle", "true");
                    CheckPuzzles();
                    NeededStartNewDialogue();
                }
            ...
            else if (game == "findobject")
            {
                ...
                swiping.SetActive(true);
                if (PlayerPrefs.GetString("need_to_complete_findobject") == "true")
                {
                    PlayerPrefs.SetString("need_to_complete_findobject", "false");
                    all_hints[4].SetActive(false);
                    all_hints[0].SetActive(false);
                    PlayerPrefs.SetString("opened_findobject", "true");
                    CheckPuzzles();
                    NeededStartNewDialogue();
                }
    ...
    public void HideRiddles(string this_text)
    {
        if (this_text == "findsibling")
        {
            PlayerPrefs.SetString("need_to_complete_findsibling", "true");
        }
        else if (this_text == "followingpuzzle")
        {
            all_puzzles[0].GetComponent<Outline>().enabled = false;
            all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[1].GetComponent<Outline>().enabled = true;
            all_puzzles[1].GetComponent<ClickEvent>().enabled = true;
            PlayerPrefs.SetString("need_to_complete_followingpuzzle", "true");
        }
        else if (this_text == "plates")
        {
            all_puzzles[0].GetComponent<Outline>().enabled = false;
            all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[1].GetComponent<Outline>().enabled = false;
            all_puzzles[1].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[2].GetComponent<Outline>().enabled = true;
            all_puzzles[2].GetComponent<ClickEvent>().enabled = true;
            PlayerPrefs.SetString("need_to_complete_plates", "true");

        }
        else if (this_text == "findobject")
        {
            all_puzzles[0].GetComponent<Outline>().enabled = false;
            all_puzzles[0].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[1].GetComponent<Outline>().enabled = false;
            all_puzzles[1].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[2].GetComponent<Outline>().enabled = false;
            all_puzzles[2].GetComponent<ClickEvent>().enabled = false;
            all_puzzles[3].GetComponent<Outline>().enabled = true;
            all_puzzles[3].GetComponent<ClickEvent>().enabled = true;
            PlayerPrefs.SetString("need_to_complete_findobject", "true");

        }

    }
    public void NeededStartNewDialogue()
    {
